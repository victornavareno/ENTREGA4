...
void MaestroSondeo(interface_t *interfaz, unsigned char mac_origen[6], unsigned char mac_destino[6], unsigned char tipo[2])
{
    ...
    while (control != 4 && !fin)
    {
        ...
        // ENVIAMOS Y MOSTRAMOS LA TRAMA CONTROL DE RESPUESTA ACK
...
void EsclavoSondeo(interface_t *interfaz, unsigned char mac_origen[6], unsigned char mac_destino[6], unsigned char tipo[2], unsigned char direccion, unsigned char control, unsigned char numeroTrama)
{
    ...
    while (control != 6)
    {
        EnviarTramaControl(interfaz, mac_origen, mac_destino, tipo, direccion, 4, numTramaEnviada); // CONTROL 4: EOT

        RecibirTramaControl(interfaz, direccion, control, numeroTrama); // RECIBIMOS TRAMAS HASTA RECIBIR CONTROL 6:  ACK - CONFIRMACION

        if (control == 21 || control == 6) // 21: NACK - 6: ACK
        {
            MostrarTrama('R', direccion, control, numeroTrama, ' ');
            cout << endl;
        }
